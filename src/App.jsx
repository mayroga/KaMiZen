...
export default function App() {
  ...
  const playAudio = () => {
    ...
    audio.loop = true;
    audio.volume = 0.3;
    audio.play().catch(() => {}); // evita error en móviles que bloquean autoplay
  };
  ...
  const generarPDF = async () => {
    try {
      ...
      data.microacciones_completadas.forEach((m, i) => {
        doc.text(`- ${m}`, 10, 60 + i * 10);
      });
  ...
  return (
    <div style={{
      ...
      height: "100vh",
      color: "white",
      backgroundColor: vital.estado_vital === "Fuego" ? "#4a0000" : "#001a33",
      padding: 20
    }}>
      ...
      <footer style={{ position: "absolute", bottom: 20, width: "100%", textAlign: "center" }}>
        <button onClick={generarPDF} style={{ padding: "10px 30px", borderRadius: 20, cursor: "pointer" }}>
